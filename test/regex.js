...
testSuite('regex', function (assert) {
  ...
  testSuite('string.replace non-match with global', function() {
    var s = '\f\n\r\t\v\xA0abc\xA0\v\t\r\n\f';
    var r = /[\\"\$\x00-\x1F\u007F-\uFFFF]/g;
    var replaced = s.replace(r, function(char) {return '';});
    assert('', replaced === 'abc');
  });

  testSuite('regex.test',  function() {
    assert('/^(true|false)$/.test(\'true\')', /^(true|false)$/.test('true') === true);
    assert('/^(true|false)$/.test(\'abc\')', /^(true|false)$/.test('abc') === false);
    assert('/^(true|false)$/.test(null)', /^(true|false)$/.test(null) === false);
  });
});
